...
Create `libs/ghost-ai-db/prisma/models/project.prisma`.
...
- timestamps
- inexes on owner ID and creation date
...
Create `libs/ghost-ai-db/src/lib/prisma.ts` as a cached singleton.

Branch by `DATABASE_URL`;

- if it starts with `prisma+postgres://`, use Accelerate
- otherwise user direct `@prisma/adapter-pg`
...
- `libs/ghost-ai-db/src/lib/prisma.ts` exports one cached Prisma instance
